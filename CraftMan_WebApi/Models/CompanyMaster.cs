...
using CraftMan_WebApi.Interface;
using System.Data;
...
namespace CraftMan_WebApi.Models
{
    ...
    public class CompanyMaster : ICompanyMaster
    {
        ...
        public   CompanyMaster GetCompanyDetail(string user)
        {
            try
            {
            ...
            {
                // Log exception
                Logger.Error("An error occurred while fetching company details", ex);
                throw new ApplicationException("An error occurred while fetching company details.", ex);
            }
        }

        public   int GetTotalcnt(string user)
        {
            //try
            //{
            //    int cnt = 0;
            ...
            try
            {
                Logger.Info("Fetching total count for user: " + user);
                DBAccess db = new DBAccess();
                db.AddParameter("@Username", user);
                object result = db.ExecuteScalar("sp_GetTotalcnt", CommandType.StoredProcedure);
                //using (SqlCommand cmd = new SqlCommand("sp_GetTotalcnt", db.strConnectionString))
                //{ cmd.CommandType = CommandType.StoredProcedure;
                //    cmd.Parameters.AddWithValue("@Username", user); 

                //    SqlDataReader reader = cmd.ExecuteReader();
                //    int cnt = 0; 
                //    if (reader.Read()) { cnt = (int)reader["totaljobrequest"]; } 
                //    reader.Close(); return cnt;
                //
                //retu
                //
                return 1; }
            ...
            {
                // Log exception
                Logger.Error("An error occurred while fetching total count", ex);
                throw new ApplicationException("An error occurred while fetching total count.", ex);
            }
        }
        ...
        public Response ValidateCompany(CompanyMaster _Company)
        {
            ...
            {
                // Log exception
                Logger.Error("An error occurred while validating company", ex);
                throw new ApplicationException("An error occurred while validating company.", ex);
            }
        }

        public   int InsertCompany(CompanyMaster _Company)
        {
            try
            {
            ...
            {
                // Log exception
                Logger.Error("An error occurred while inserting company", ex);
                throw new ApplicationException("An error occurred while inserting company.", ex);
            }
        }
